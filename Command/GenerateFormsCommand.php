...
class GenerateFormsCommand extends ContainerAwareCommand {
	...
	protected function configure() {
		parent::configure();
		$this->setName('webit:extjs:generate:forms')
			->addArgument('bundle_name',InputArgument::REQUIRED)
					->setDescription('Generate ExtJS forms (from entities and documents) for given Bundle');
	}
	...
	private function getExtJsClassDir($extjsSubclass) {
		...
		$dir = $this->bundle->getPath() .'/Resources/public' . $this->getContainer()->getParameter('webit_ext_js.js_root_dir') . '/app/view/form/';
	...
	private function renderModel($phpClass,$extjsSubclass) {
		...
		$view->setTemplate('WebitExtJsBundle:_commandGenerateForms:form.js.twig');
	...
	private function getDataForClass($class) {
		...
		foreach($cm->getFieldNames() as $fieldName) {
			...
			$arField = FormElementsMap::getFormElementDefinition($fieldName, $arField['type']);
			$arFields[] = $arField;
		}
